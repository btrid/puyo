// https://gist.github.com/yat1ma30/6327701#file-main-js
enchant();  // 初期化
    ...
    game.onload = function () {	// ゲームが開始された時の関数をセット
        ...
        for (var i=0; i<field.length; i++) {
            ...
            for (var j=0; j<MAX_COL; j++) {
                if (j==0 || j==MAX_COL-1 || i==MAX_ROW-1) temp_array[j] = FIELD_BLOCK; // ブロック(壁)を配置
                else temp_array[j] = FIELD_SPACE; // 空
            }
            field[i] = temp_array;
        }
...
function countPuyos (row, col, field) {
    ...
    field[row][col] = FIELD_SPACE; // この場所をチェックした証として一時的に空白に
